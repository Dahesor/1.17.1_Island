...
execute unless score #version player_version matches 9.. run scoreboard players set #version player_version 9
execute as @a unless score @s player_version = #version player_version run function change_log

execute as @a unless score @s leave_game matches ..-1 run function operation:updater/initialize
execute if entity @a[scores={leave_game=0..}] run schedule function hidden/self_check 5t
